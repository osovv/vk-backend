...
class Server:
    def __init__(self, host, port, default_media_path):
        self.host = host
        self.port = port
        self.media_urls = {i: default_media_path for i in range(1, 7)}
    ...
    def update_media_url(self, number):
        number = int(number)
        if type(number) == int and 1 <= number <= 6:
            request_body = dict(request.json)
            new_url = request_body.get('url', None)
            self.media_urls[number] = new_url
            return f'Successfully updated url for screen {number}', 200
        else:
            abort(404, description='Screen number is int between 1 and 6.')
